...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . $link . '">' . $aRow['transactionid'] . '</a>';

    $numberOutput .= '<div class="row-options">';
    $numberOutput .= '<a href="javascript:void(0)" data-toggle="modal" data-target="#viewPaymentsModal" data-transaction-id="' . $aRow['transactionid'] . '" data-date="' . substr($aRow['date'], 0, 10) . '">' . _l('view') . '</a>';
    if (has_permission('payments', '', 'edit')) {
        $numberOutput .= ' | <a href="javascript:void(0)" onclick="edit_payment_modal(' . $aRow['id'] . '); return false;">' . _l('edit') . '</a>';
    }
    if ($hasPermissionDelete) {
        $numberOutput .= ' | <a href="' . admin_url('payments/delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    if ($aRow['invoiceid'] == 0) {
        $numberOutput .= ' | <a href="javascript:void(0)" data-toggle="modal" data-target="#attachInvoiceModal" data-id="' . $aRow['id'] . '" data-client-id="' . ($aRow['clientid'] ? $aRow['clientid'] : $query[0]['cid']) . '">' . _l('Invoice') . '</a>';
    }
    $numberOutput .= ' | <a href="javascript:void(0)" data-toggle="modal" data-target="#noteModal" data-id="'.$aRow['id'].'">' . _l('Add Note') . '</a>';
    $numberOutput .= '</div>';
