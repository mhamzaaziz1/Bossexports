...
foreach ($rResult as $aRow) {
    ...
    // -------------------------------------------------------------------------
    // CUSTOM LOGIC: Waterfall Calculation
    // Priority: 1-30 -> 31-60 -> 61-90 -> Over 90 -> Current
    // -------------------------------------------------------------------------
    
    // 1. Get the initial Total Balance
    $remaining_balance = (float)$aging['current'];

    // --- STEP 1: Check 1-30 Days ---
    $bucket_val = (float)$aging['1_30'];
    if ($remaining_balance > 0) {
        if ($remaining_balance < $bucket_val) {
            $days_1_30 = $remaining_balance;
            $remaining_balance = 0;
        } else {
            $days_1_30 = $bucket_val;
            $remaining_balance = $remaining_balance - $bucket_val;
        }
    } else {
        $days_1_30 = 0;
    }

    // --- STEP 2: Check 31-60 Days ---
    $bucket_val = (float)$aging['31_60'];
    if ($remaining_balance > 0) {
        if ($remaining_balance < $bucket_val) {
            $days_31_60 = $remaining_balance;
            $remaining_balance = 0;
        } else {
            $days_31_60 = $bucket_val;
            $remaining_balance = $remaining_balance - $bucket_val;
        }
    } else {
        $days_31_60 = 0;
    ...
    // --- STEP 3: Check 61-90 Days ---
    $bucket_val = (float)$aging['61_90'];
    if ($remaining_balance > 0) {
        if ($remaining_balance < $bucket_val) {
            $days_61_90 = $remaining_balance;
            $remaining_balance = 0;
        } else {
            $days_61_90 = $bucket_val;
            $remaining_balance = $remaining_balance - $bucket_val;
        }
    } else {
        $days_61_90 = 0;
    ...
    // --- STEP 4: Check Over 90 Days ---
    $bucket_val = (float)$aging['over_90'];
    // For Over 90, we can also use the same logic, or dump remainder here.
    // Given the priority order, we check it now.
    if ($remaining_balance > 0) {
    ...
    // --- STEP 5: Check Current (Last priority) ---
    // Any remaining balance goes here.
    ...
    $extend='';
    $row[] = '<div class="text-center">' . number_format($aging['current'], 2) . '</div>';
    ...
    $row[] = '<div class="text-center"><strong>' . number_format($aging['current'], 2) . '</strong></div>';
