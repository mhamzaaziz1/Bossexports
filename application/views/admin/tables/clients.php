...
// 0 as client_balance replaced with conditional:
            ($this->ci->input->post('length') == -1 ? '(SELECT (
                SELECT COALESCE(SUM(total),0) 
                FROM ' . db_prefix() . 'invoices 
                WHERE clientid=' . db_prefix() . 'clients.userid AND status != 5 AND status != 6
              ) - (
                ...
                WHERE ' . db_prefix() . 'invoices.clientid=' . db_prefix() . 'clients.userid AND ' . db_prefix() . 'invoices.status != 5 AND ' . db_prefix() . 'invoices.status != 6
              )) as client_balance' : '0 as client_balance'),
        ];
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
            ...
            if ($this->ci->input->post('length') == -1) {
                ...
                $row[] = app_format_money($aRow['client_balance'], $base_currency);
            } else {
                $row[] = '<a href="javascript:void(0);" class="text-center display-block client-balance-check" data-id="' . $aRow['userid'] . '"><i class="fa fa-eye"></i></a>';
            }
