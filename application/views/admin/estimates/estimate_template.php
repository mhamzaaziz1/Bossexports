...
$("#caldata").click(function(){
        ...
        for( var i=1; i<=500; i++) {
            ...
            if(Number($('[name="newitems['+i+'][custom_fields][items][5]"]').val())>=0){
                vol=Number($('[name="newitems['+i+'][custom_fields][items][5]"]').val());
            }else{
                vol=0;
            }
            volume =volume + vol;
            if(Number($('[name="newitems['+i+'][custom_fields][items][1]"]').val())>=0){
                wei=Number($('[name="newitems['+i+'][custom_fields][items][1]"]').val());
            }else{
                wei=0;
            }
            weight =weight + wei;
            if(Number($('[name="items['+i+'][custom_fields][items][5]"]').val())>=0){
                volit=Number($('[name="items['+i+'][custom_fields][items][5]"]').val());
            }else{
                volit=0;
            }
            volume =volume + volit;
            if(Number($('[name="items['+i+'][custom_fields][items][1]"]').val())>=0){
    ...
    $("#adjust").change(function(){
            $("#adj").val($("#adjust option:selected").val());
            calculate_total();
        });
</script>
