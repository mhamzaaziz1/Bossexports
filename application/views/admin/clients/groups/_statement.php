           ...
           <td>
            ...
            if(isset($data['invoice_id'])) {
                if($data['duedate']=="" || $data['duedate']==NULL ){
                    ...
                    $CI->db->where('id',$data['invoice_id']);
                    $query = $CI->db->get()->result();
                echo '<a href="'.admin_url('expenses/list_expenses/'.$data['invoice_id']).'" target="_blank">Expense (#'.$data['invoice_id'].')<br></a>'.$query[0]->note;
        }
        else{
            echo _l('statement_invoice_details',array('<a href="'.admin_url('invoices/list_invoices/'.$data['invoice_id']).'" target="_blank">'.format_invoice_number($data['invoice_id']).'</a>',_d($data['duedate'])));
                ...
                if(!empty($data['payment_invoice_id'])){
                    ...
                    $CI->db->from('tblinvoicepaymentrecords');
                     $CI->db->where('id',$data['payment_id']);
                     $query = $CI->db->get()->result();
