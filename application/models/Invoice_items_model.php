...
class Invoice_items_model extends App_Model
{
    ...
    {
        $this->db->select('a.commodity_code,a.rate, a.id, a.description as name, a.commodity_code as subtext ');
        $this->db->where('a.isactive', 1);
        $this->db->like('a.description', $q);
        $this->db->or_like('a.long_description', $q);
        $this->db->or_like('a.commodity_code', $q);
        $this->db->where('a.isactive', 1);
        ...
        foreach ($items as $key => $item) {
            $this->db->select('SUM(b.qty) as pqty');
            $this->db->from(db_prefix() . 'itemable as b');
            $this->db->join(db_prefix() . 'proposals'  , 'tblproposals.id = b.rel_id' );
            $this->db->where('b.description', $item['name']);
            ...
            $this->db->where('c.description', $item['name']);
            ...
            $this->db->where('d.description', $item['name']);
