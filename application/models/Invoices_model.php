...
class Invoices_model extends App_Model
{
    ...
    public function get_unpaid_invoices($id)
    {
        ...
        $this->db->select(db_prefix() . 'invoices.id as id,concat(prefix,number) as pur_order_number,' . get_sql_select_client_company());
     ...
     public function sumcustom($id='')
    {
      $this->db->select("tblcustomfields.id , name");
      $this->db->from('tblcustomfields');
      $this->db->where('tblcustomfields.fieldto','items');
      $query = $this->db->get();
      return $query->result();
    }
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                ...
                $invoice->items_full       = est_get_items_by_type('invoice', $id);
    ...
    public function mark_as_paid($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_invoice_activity($id, 'invoice_activity_marked_as_paid');

            hooks()->do_action('invoice_marked_as_paid', $id);
