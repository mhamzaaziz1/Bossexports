...
class Authentication_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($staff == true) {
                ...
                if (!$twoFactorAuth) {
                    hooks()->do_action('before_staff_login', [
                        'email'  => $email,
                        'userid' => $_id === 'staffid' ? $user->staffid : $user->id,
                    ]);

                    $user_data = [
                        'staff_user_id'   => $_id === 'staffid' ? $user->staffid : $user->id,
            ...
            if (!$twoFactorAuth) {
                if ($remember) {
                    $this->create_autologin($staff ? $user->staffid : $user->id, $staff);
                }

                $this->update_login_info($staff ? $user->staffid : $user->id, $staff);
            } else {
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            $new_pass_key = app_generate_hash();
            $user_id = $staff ? $user->staffid : $user->id;
            $this->db->where($_id, $user_id);
            $this->db->update($table, [
                'new_pass_key'           => $new_pass_key,
                'new_pass_key_requested' => date('Y-m-d H:i:s'),
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($staff == false) {
                    $sent = send_mail_template('customer_contact_forgot_password', $user->email, $user->userid, $user_id, $data);
                } else {
                    $sent = send_mail_template('staff_forgot_password', $user->email, $user_id, $data);
                }
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $merge_fields = [];
            if ($staff == false) {
                $sent = send_mail_template('customer_contact_password_resetted', $user->email, $user->userid, $user->id);
            } else {
                $sent = send_mail_template('staff_password_resetted', $user->email, $user->staffid);
            }
