...
class Payments_model extends App_Model
{
    ...
    public function update($data, $id)
    {
        ...
        $payment      = $this->get($id);
        $updated      = false;
        $data1['date'] = to_sql_date($data['date']);
        $data1['note'] = $data['note'];
        $tid            =       $data['transactionid'];
        $data = hooks()->apply_filters('before_payment_updated', $data1, $id);

        $this->db->where('transactionid', $tid);
        $this->db->update('invoicepaymentrecords', $data1);
