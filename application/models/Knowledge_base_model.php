...
class Knowledge_base_model extends App_Model
{
     ...
     */
    public function get_kbg($id = '', $active = '', $date_filter = '')
    {
        ...
        $groups = $this->db->get(db_prefix() . 'knowledge_base_groups')->result_array();

        // If date filter is provided, apply it to the articles query
        if (!empty($date_filter)) {
            foreach ($groups as $key => $group) {
                // Get articles for this group with date filter
                $this->db->select('articleid, subject, description, staff_article, datecreated, article_order, active, slug, group_id, long_description');
                $this->db->where('group_id', $group['groupid']);
                $this->db->where('active', 1);
                $this->db->where($date_filter, null, false);
                $this->db->order_by('article_order', 'asc');
                $articles = $this->db->get(db_prefix() . 'knowledge_base')->result_array();

                $groups[$key]['articles'] = $articles;
            }
        } else {
