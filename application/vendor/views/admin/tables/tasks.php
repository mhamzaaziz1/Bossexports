...
foreach ($rResult as $aRow) {
    ...
    if ($aRow['rel_name']) {
        ...
        $this->ci->db->join(db_prefix() . 'clients'  , 'tblclients.userid = estimates.clientid' );
        $this->ci->db->where('estimates.id', $aRow['rel_id']);
        $e=$this->ci->db->get()->result();
        $outputName .= '<br><span class="hide"> - </span><a class="text-muted task-table-related" data-toggle="tooltip" title="Client" href="' . $link . '">' . $e[0]->company . '</a>';
    }
