...
$aColumns = [
    ...
    '(SELECT status FROM ' . db_prefix() . 'tasks WHERE rel_id = ' . db_prefix() . 'estimates.id Limit 1) as tstatus',
...
foreach ($rResult as $aRow) {
    ...
    $this->ci->db->select('*');
        $this->ci->db->from('tblestimates');
        $this->ci->db->where('tblestimates.id', $aRow['id']);
        $estimate = $this->ci->db->get()->result_array();
    
    $row[] = app_format_money($estimate[0]['total']-($estimate[0]['total_tax']-$estimate[0]['discount_total']+$estimate[0]['subtotal']), $aRow['currency_name']);
    ...
    $row[] = format_task_status($aRow['tstatus']);
