...
$info_right_column .= '<span style="font-weight:bold;font-size:24px;">' . _l('Credit Note') . '</span><br />';
// Add logo
...
// Bill to
$credit_note_info = '<b style="color:#424242; font-size:13px">' . _l('To') . ':</b><br>';
$credit_note_info .= '<div style="font-size:14px; color:#424242;" border="1px">';
    $credit_note_info .= format_customer_info($credit_note, 'credit_note', 'billing');
$credit_note_info .= '</div>';

// ship to to
$credit_note_info1 .= '<div style="font-size:14px" border="1px"> Credit Note # <b style="color:#4e4e4e;">' . $credit_note_number.'</b>';

$credit_note_info1 .= '<br />' . _l('Date') . ' ' . _d($credit_note->date) . '<br>';

// if (!empty($credit_note->duedate)) {
//     $credit_note_info1 .= _l('estimate_data_duedate') . ' ' . _d($credit_note->duedate) . '<br />';
// }
$credit_note_info1 .= '<b><u>Payment Accounts</u></b><br><b>FNB:</b> Branch : 250505 Ac No: 62025537450<br>
...
if (true) {
    $credit_note_info1 .= '<b style="font-size:14px">' . _l('ship_to') . ':</b><br>';
    $credit_note_info1 .= '<div style="font-size:14px; color:#424242;" border="1px">';
    $credit_note_info1 .= format_customer_info($credit_note, 'credit_note', 'shipping');
    $credit_note_info1 .= '</div>';
}
$organization_info .=$credit_note_info;
$left_info  = $swap == '1' ? $credit_note_info : $organization_info;
 $right_info = $credit_note_info1;
...
// $tblhtml = "";
  ...
  $qty=0;
  // The items table
  foreach ($credit_note->items as $key ) {
    $tbltotal .='<tr>';
    ...
    $CI->db->where('tblitemable.rel_id',$credit_note->id);
    ...
    $CI->db->where('tblitem_tax.rel_id',$credit_note->id);
...
$sum_custom = $CI->invoices_model->sumcustom($credit_note->id);
...
foreach ($sum_custom as $custom){
  ...
  $tbltotal .= '<tr>
      ...
      $tbltotal .= '<td align="right" width="55%"><strong>' . _l('estimate_subtotal') . '</strong></td>
         <td align="right" width="15%">' . app_format_money($credit_note->subtotal/$maxi, $credit_note->currency_name) . '</td>';
      }else{
          $taxto=0;
        foreach ($items->taxes() as $tax) {
            ...
            <td align="right" width="15%">' . app_format_money($tax['total_tax']/$maxi, $credit_note->currency_name) . '</td>';
        }
      }
      $tbltotal .= '</tr>';
...
if (is_sale_discount_applied($credit_note)) {
    ...
    if (is_sale_discount($credit_note, 'percent')) {
        $tbltotal .= '(' . app_format_number($credit_note->discount_percent/$maxi, true) . '%)';
    }
    $tbltotal .= '</strong>';
    $tbltotal .= '</td>';
    $tbltotal .= '<td align="right" width="15%">-' . app_format_money($credit_note->discount_total/$maxi, $credit_note->currency_name) . '</td>
    </tr>';
}
...
if (false) {
    ...
    <td align="right" width="15%">' . app_format_money(($credit_note->total-($taxto-$credit_note->discount_total+$credit_note->subtotal))/$maxi, $credit_note->currency_name) . '</td>
</tr>';
}
$tbltotal .= '
<tr style="background-color:#f0f0f0;">
    <td align="left" width="30%"><strong>Total Items=</strong>' . $qty . '</td>
    <td align="right" width="55%"><strong>' . _l('estimate_total') . '</strong></td>
    <td align="right" width="15%">' . app_format_money(($credit_note->total/$maxi), $credit_note->currency_name) . '</td>
</tr>';
if (count($credit_note->payments) > 0 && get_option('show_total_paid_on_estimate') == 1) {
    $tbltotal .= '
    <tr>
        ...
        'field' => 'amount',
        'where' => [
            'estimateid' => $credit_note->id,
        ],
    ])/$maxi, $credit_note->currency_name) . '</td>
    </tr>';
}

if (get_option('show_credits_applied_on_estimate') == 1 && $credits_applied = total_credits_applied_to_estimate($credit_note->id)) {
    $tbltotal .= '
    <tr>
        <td align="right" width="85%"><strong>' . _l('applied_credits') . '</strong></td>
        <td align="right" width="15%">-' . app_format_money($credits_applied/$maxi, $credit_note->currency_name) . '</td>
    </tr>';
}

if (get_option('show_amount_due_on_estimate') == 1 && $credit_note->status != estimates_model::STATUS_CANCELLED) {
    $tbltotal .= '<tr style="background-color:#f0f0f0;">
       <td align="right" width="85%"><strong>' . _l('estimate_amount_due') . '</strong></td>
       <td align="right" width="15%">' . app_format_money($credit_note->total_left_to_pay/$maxi, $credit_note->currency_name) . '</td>
   </tr>';
}
...
if (count($credit_note->payments) > 0 && get_option('show_transactions_on_estimate_pdf') == 1) {
    $pdf->Ln(4);
    ...
    $tblhtml .= '<tbody>';
    foreach ($credit_note->payments as $payment) {
        ...
        $tblhtml .= '
            ...
            <td>' . app_format_money($payment['amount'], $credit_note->currency_name) . '</td>
            </tr>
        ';
    }
    $tblhtml .= '</tbody>';
