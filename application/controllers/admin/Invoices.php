...
class Invoices extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        $this->list_sinvoices($id);
        }else{
        $this->list_invoices($id);
        }
    }
    public function allpaid($id = '')
    {
        $this->db->update('tblinvoices',array('status' => 2));
    ...
    /*List split invoices*/
    public function list_sinvoices($id = '')
    {
        if (!has_permission('invoices', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $data['invoices_statuses']    = $this->invoices_model->get_statuses();
        $data['bodyclass']            = 'invoices-total-manual';
        $this->load->view('admin/invoices/split/split_manage', $data);
    }
    ...
    public function stable($clientid = '')
    {
        if (!has_permission('invoices', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $this->app->get_table_data('sinvoices', [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function mark_as_paid($id)
    {
        ...
        $success = $this->invoices_model->mark_as_paid($id);

        if ($success) {
            set_alert('success', _l('invoice_marked_as_paid_successfully'));
        }
