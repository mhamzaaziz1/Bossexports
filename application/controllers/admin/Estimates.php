...
class Estimates extends AdminController
{
    ...
    public function estimate($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    $data=["billable"=>"on", "name"=> "Docs", "hourly_rate"=>"0", "milestone"=>"", "startdate"=>date('Y-m-d H:i:s'), "duedate"=>"", "priority"=>"3", "repeat_every"=>"", "repeat_every_custom"=> "1", "repeat_type_custom"=>"day", "rel_type"=> "estimate", "rel_id"=>$id, "tags"=>"", "description"=>"DOCS"];
                    $tid = $this->tasks_model->add($data);
                    $subdata = [["taskid"=>$id,"description"=>"PACKING LIST"],
                    ["taskid"=>$tid,"description"=>"INVOICES"],
                    ["taskid"=>$tid,"description"=>"EDI ENTRY / EXIT"],
                    ["taskid"=>$tid,"description"=>"ROAD MANIFEST"],
                    ["taskid"=>$tid,"description"=>"SAD500"],
                    ["taskid"=>$tid,"description"=>"TRACKING"],
                    ["taskid"=>$tid,"description"=>"TRANSPORT INV"]];
                    foreach ($subdata as $i){
                        $this->tasks_model->add_checklist_item($i);
                    }
        ...
        if ($id == '') {
            ...
            if($this->input->get("proposal")!=""){
                ...
                $data['proposal']  = $this->proposals_model->get($this->input->get("proposal"));
    ...
    public function get_inventory($commodity_id){
        if($commodity_id){
		$sql = 'SELECT sum(inventory_number) as inventory_number FROM ' . db_prefix() . 'inventory_manage
		where ' . db_prefix() . 'inventory_manage.commodity_id = ' . $commodity_id . ' and warehouse_id=1  group by ' . db_prefix() . 'inventory_manage.commodity_id';
    ...
    public function convert_to_invoice($id)
    {
        ...
        $inv_detail=$this->db->select("tblitemable.* ,SUM(tblitemable.Qty) as entered_qty, tblitems.id as id")->from("tblitemable")->join('tblitems', 'tblitemable.description = tblitems.description', 'left')->group_by('tblitemable.description')->where("rel_id", $id)->where("rel_type","estimate")->get()->result_array();
        //  var_dump($this->db->last_query());die;
        
        foreach ($inv_detail as $p_check){
            // var_dump($p_check['id']);
            // var_dump($p_check['entered_qty']); die;
            if($this->get_inventory($p_check['id'])->inventory_number < $p_check['entered_qty']){
                set_alert('warning', 'Stock low, Please check Inventory for '.$p_check['description'].'');
                redirect(admin_url('estimates/list_estimates/' . $id));
                die;
            }
        }
        ...
        /* Convert estimate to invoice */
    public function release_retainer($id)
    {
        if (!$id) {
            die('No estimate found');
        }
        ...
        // var_dump($estimate);
        // die();
        $data['client_id'] = $estimate->clientid;
        $data['amount'] = ($estimate->subtotal * 0.15);
        $data['date'] = date('Y-m-d');
        $data['note'] = "Retainer Released for '.$estimate->prefix$estimate->number .'";
        $data['daterecorded'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'invoicepaymentrecords', $data);
        if ($this->db->affected_rows() > 0) {
         set_alert('success', _l('Amount Released'));
        redirect(admin_url('estimates/list_estimates/' . $id));
        }else{
            set_alert('Error', _l('Something went wrong'));
        }
    }
    ...
    public function pdf($id)
    {
        ...
        $estimate        = $this->estimates_model->get_pdf($id);
