...
get_instance()->load->helper(LINE_DISCOUNTS_MODULE_NAME . '/'.LINE_DISCOUNTS_MODULE_NAME);
...
function line_discounts_add_footer_components()
{
    ...
    $enable_negative_discount = line_discount_get_option_value('enable_negative_discount', 0);


    echo "<script> 
                var ld_discount_text = '"._l('line_discount_rate')."'; 
                var ld_tax_text = '"._l('invoice_table_tax_heading')."'; 
                var ld_enable_negative_discount = $enable_negative_discount; 
                
          </script>";

    echo '<script src="' . module_dir_url(LINE_DISCOUNTS_MODULE_NAME, 'assets/js/line_discounts.js?v=1') . '"></script>';
}
...
hooks()->add_action("admin_init", function (){
    ...
    $capabilities["capabilities"] = [
        ...
        "discount_setting"             => _l('line_discount_setting') ,

    ];

    register_staff_capabilities("discount_setting", $capabilities , _l('line_discount_setting') );

    if (staff_can('discount_setting', 'discount_setting' )) {

        $CI->app_menu->add_setup_menu_item('discount_settings', [

            'href'     => admin_url('line_discounts/setting'),

            'name'     => _l('line_discount_setting'),
