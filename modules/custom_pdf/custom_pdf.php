...
/*
    ...
    Description: Personalize your Proposals, Estimates, Invoices, Credit Notes, and Contracts PDF Easily. Waaillab.com
    Version: 1.1.2
...
$custom_pdf = ['contract', 'invoice', 'proposal', 'credit_note', 'estimate', 'payment', 'proforma', 'delivery_note'];
...
{
    hooks()->add_filter($type.'_pdf_class_path', function ($path) use ($type) {
...
hooks()->add_filter('get_upload_path_by_type', function ($path, $type) {
    switch ($type) {
        ...
        case 'custom_pdf_proforma':
            $path = FCPATH . 'uploads/custom_pdf/proforma/';
            break;

        case 'custom_pdf_delivery_note':
            $path = FCPATH . 'uploads/custom_pdf/delivery_note/';
            break;
    }
