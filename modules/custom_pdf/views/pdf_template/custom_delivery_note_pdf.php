...
$info_right_column .= '<span style="font-weight:bold;font-size:35px; "><b style="color:#0c5191;">' . _l('delivery_note_pdf_heading') . '</span><br />';
$info_right_column .= '<span style="font-weight:bold;font-size:20px; "><b style="color:#4e4e4e;"># ' . $delivery_note_number . '</b>';

if (get_option('show_status_on_pdf_ei') == 1) {
    if (function_exists('delivery_note_status_color_pdf')) {
        $info_right_column .= '<br /><span style="color:rgb(' . delivery_note_status_color_pdf($status) . ');text-transform:uppercase;">' . format_delivery_note_status($status, '', false) . '</span>';
         ...
         // Fallback or just display text if function not available
        $info_right_column .= '<br /><span style="text-transform:uppercase;">' . format_delivery_note_status($status, '', false) . '</span>';
    }
}
...
// The items table
// Use customPdfItemsTableData to get the custom table class
$items = customPdfItemsTableData($delivery_note, 'delivery_note', 'pdf');
...
// Apply custom colors to total row
$total_bg_color = !empty(getPdfOptions('delivery_note', 'items_table', 'total_row_bg_color')) ? getPdfOptions('delivery_note', 'items_table', 'total_row_bg_color') : "#0c5191";
$total_text_color = !empty(getPdfOptions('delivery_note', 'items_table', 'total_row_text_color')) ? getPdfOptions('delivery_note', 'items_table', 'total_row_text_color') : "#ffffff";

$tbltotal .= '
<tr style="background-color:' . $total_bg_color . ';">
    <td style="color:' . $total_text_color . ';" align="right" width="85%"><strong>' . _l('delivery_note_total') . '</strong></td>
    <td style="color:' . $total_text_color . ';" align="right" width="15%">' . $total . '</td>
</tr>';
