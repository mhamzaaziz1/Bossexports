...
if (!function_exists('customPdfItemsTableData')) {
    function customPdfItemsTableData($transaction, $type, $for = 'html', $admin_preview = false)
    {
        ...
        return hooks()->apply_filters('items_table_class', $class, $transaction, $type, $for, $admin_preview);
    }
}
...
if (!function_exists('get_custom_pdf_items_table_styling_areas')) {
    ...
    {
        $types = ['proposals', 'estimate', 'invoice', 'credit_note', 'payment', 'proforma', 'delivery_note'];
...
if (!function_exists('get_custom_pdf_items_table_applied_styling_area')) {
    function get_custom_pdf_items_table_applied_styling_area()
    {
        ...
        // Get the applied custom table styling area values
        $proforma_table_style = get_option('proforma_item_table_custom_style');
        if ('' == $proforma_table_style) {
            // return [];
        }
        $proforma_table_style = json_decode($proforma_table_style);

        // Get the applied custom table styling area values
        $delivery_note_table_style = get_option('delivery_note_item_table_custom_style');
        if ('' == $delivery_note_table_style) {
            // return [];
        }
        $delivery_note_table_style = json_decode($delivery_note_table_style);

        return array_merge($proposals_table_style ?? [], $estimate_table_style ?? [], $invoice_table_style ?? [], $credit_note_table_style ?? [], $payment_table_style ?? [], $proforma_table_style ?? [], $delivery_note_table_style ?? []);
    }
}
