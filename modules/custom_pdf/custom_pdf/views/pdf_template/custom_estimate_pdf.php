...
$info_right_column .= '<span style="font-weight:bold;font-size:35px; "><b style="color:#0c5191;">' . _l('estimate_pdf_heading') . '</span><br />';
$info_right_column .= '<span style="font-weight:bold;font-size:20px; "><b style="color:#4e4e4e;"># ' . $estimate_number . '</b>';
...
if (!empty($estimate->clientnote)) {
    ...
    $pdf->Cell(0, 0, _l(''), 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', (''), 0, 1, false, true, 'L', true);
}

if (!empty($estimate->clientnote)) {
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('Dear Sir,'), 0, 3, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', ('We thank you for your enquiry and are pleased to submit our proposal for the supply of the following items.'), 0, 1, false, true, 'L', true);
}
