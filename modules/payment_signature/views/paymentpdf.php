...
// START PAYMENT SIGNATURE MODULE CODE
if (get_option('payment_signature_enable') == 1) {
    $pdf->Ln(20);
    $signaturePath = get_upload_path_by_type('company') . get_option('payment_signature_image_path');
    $stampPath = get_upload_path_by_type('company') . get_option('payment_stamp_image_path');
    $sigText = get_option('payment_signature_text');
    
    // Check if files exist to avoid broken images
    $showSignature = !empty(get_option('payment_signature_image_path')) && file_exists($signaturePath);
    $showStamp = !empty(get_option('payment_stamp_image_path')) && file_exists($stampPath);
    
    // We use actual paths for TCPDF usually, but let's try URL first as it is safer for HTML src if allowed
    // Actually for TCPDF writeHTML, absolute server paths are better for performance and consistency with local files
    
    $signatureSrc = $showSignature ? $signaturePath : '';
    $stampSrc = $showStamp ? $stampPath : '';
    
    $tblhtml = '<table width="100%" border="0" cellpadding="5">';
    $tblhtml .= '<tr>';
    
    // Stamp column
    $tblhtml .= '<td width="50%" valign="bottom">';
    if ($showStamp) {
        $tblhtml .= '<img src="' . $stampSrc . '" width="150" />';
    }
    $tblhtml .= '</td>';
    
    // Signature column
    $tblhtml .= '<td width="50%" align="right" valign="bottom">';
    if ($sigText != '') {
        $tblhtml .= '<span style="font-weight:bold; font-size: 1.1em;">' . $sigText . '</span><br/><br/>';
    }
    if ($showSignature) {
        $tblhtml .= '<img src="' . $signatureSrc . '" width="400px" />';
    }
    $tblhtml .= '</td>';
...
// END PAYMENT SIGNATURE MODULE CODE
