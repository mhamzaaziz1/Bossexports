...
return (new App_table('vendors'))
    ->outputUsing(function ($params) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $CI = & get_instance(); // Using this->ci in closure context but ensuring instance is valid
            // Re-use $this->ci since we are in the App_table class context (outputUsing binds this)
            // But the original code used $CI->db. Let's use $this->ci->db where possible or $CI
            
            if ($this->ci->input->post('length') == -1) {
                // Calculate balance for export/all view
                $result['invoiced_amount'] = $this->ci->db->query('SELECT
                SUM(' . db_prefix() . 'pur_orders.total) as invoiced_amount
                FROM ' . db_prefix() . 'pur_orders
                WHERE vendor = ' . $aRow['userid'] .' and returns=0')->row()->invoiced_amount;

                if ($result['invoiced_amount'] === null) {
                    $result['invoiced_amount'] = 0;
                }
                
                $result['invoiced_amount1'] = $this->ci->db->query('SELECT
                SUM(' . db_prefix() . 'pur_orders.total*-1) as invoiced_amount
                FROM ' . db_prefix() . 'pur_orders
                WHERE vendor = ' . $aRow['userid'] .' and returns=1')->row()->invoiced_amount;
                ...
                WHERE ' .  db_prefix() . 'pur_orders.vendor = ' . $aRow['userid'])->row()->amount_paid;
                
                $result['amount_paid1'] = $this->ci->db->query('SELECT
                SUM(' . db_prefix() . 'pur_order_payment.amount) as amount_paid
                FROM ' . db_prefix() . 'pur_order_payment
                WHERE tblpur_order_payment.bulk=0 and tblpur_order_payment.pur_order=0 and tblpur_order_payment.vendor = ' . $aRow['userid'])->row()->amount_paid;
                
                $result['amount_paid2'] = $this->ci->db->query('SELECT
                SUM(' . db_prefix() . 'pur_order_payment.amount) as amount_paid
                FROM ' . db_prefix() . 'pur_order_payment
                WHERE tblpur_order_payment.bulk=1 and tblpur_order_payment.pur_order=0 and tblpur_order_payment.vendor = ' . $aRow['userid'])->row()->amount_paid;
                ...
                $result['amount_paid']=$result['amount_paid1']+$result['amount_paid2']+$result['amount_paid'];
                $result['beginning_balance']=0;
                
                $vendor_row = $this->ci->db->select("balance")->from(db_prefix().'pur_vendor')->where('userid', $aRow['userid'])->get()->row();
                ...
                $row[] = app_format_money($result['balance_due'], get_base_currency());
            } else {
                $row[] = '<a href="javascript:void(0);" class="text-center display-block vendor-balance-check" data-id="' . $aRow['userid'] . '"><i class="fa fa-eye"></i></a>';
            }
