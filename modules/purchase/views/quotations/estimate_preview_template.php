   ...
   <div class="panel_s">
      <div class="panel-body">
         <?php if($estimate->status == 1){ ?>
           <div class="ribbon info"><span class="fontz9"><?php echo "Open"; ?></span></div>
       <?php }elseif($estimate->status == 2){ ?>
         <div class="ribbon success"><span><?php echo "Close"; ?></span></div>
       <?php }elseif($estimate->status == 3){ ?>  
         <div class="ribbon danger"><span><?php echo "Rejected"; ?></span></div>
       <?php } ?>
            ...
            <div class="col-md-8">
             ...
             <div class="btn-group pull-right">
                  ...
                  <ul class="dropdown-menu dropdown-menu-right">
                     <li class="hidden-xs"><a href="<?php echo admin_url('purchase/pur_request_pdf/'.$estimate->id.'?output_type=I'); ?>">View PDF</a></li>
                     <li class="hidden-xs"><a href="<?php echo admin_url('purchase/pur_request_pdf/'.$estimate->id.'?output_type=I'); ?>" target="_blank">View PDF in New Tab</a></li>
                     <li><a href="<?php echo admin_url('purchase/pur_request_pdf/'.$estimate->id.'?output_type=I'); ?>">Download</a></li>
                     <li>
                        <a href="<?php echo admin_url('purchase/pur_request_pdf/'.$estimate->id.'??print=true'); ?>" target="_blank">Print</a>
                     </li>
                  </ul>
               </div>
               ...
               <select name="status" id="status" class="selectpicker pull-right mright10" onchange="change_status_pur_estimate(this,<?php echo html_entity_decode($estimate->id); ?>); return false;" data-live-search="true" data-width="35%" data-none-selected-text="<?php echo _l('change_status_to'); ?>">
                 <option value=""></option>
                 <option value="1" class="<?php if($estimate->status == 1) { echo 'hide';}?>"><?php echo "Open"; ?></option>
                 <option value="2" class="<?php if($estimate->status == 2) { echo 'hide';}?>"><?php echo "Close"; ?></option>
         ...
         <?php
         $CI =& get_instance();
         $_data = $CI->db->query('select tblpur_orders.pur_order_number as pur, tblpur_orders.id from tblpur_orders INNER JOIN tblpur_estimates ON tblpur_orders.estimate = tblpur_estimates.id  where tblpur_estimates.id ='.$estimate->id)->row();
         ?>
         <span>Purchase  related to this order <a href="<?php echo admin_url('purchase/purchase_order/'.$_data->id)?>"><?php echo $_data->pur ?></span></a>
