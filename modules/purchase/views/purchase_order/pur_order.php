                   ...
                   <div class="col-md-6">
                      <div class="row">
                        <div class="col-md-4">
                          ...
                          echo render_input('pur_order_name','pur_order_number',$pur_order_name); ?>
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="switch">
        					    <div class="switch">
                                  <label>
                                    <input type="checkbox" name="waiting">
                      ...
                      <div class="row">
                        <div class="col-md-<?php if(get_purchase_option('purchase_order_setting') == 1 ){ echo '12' ;}else{ echo '6' ;} ;?>">
                          <?php $prefix = get_purchase_option('pur_order_prefix');
                                ...
                                $next_number1 = date("Y");
                                $next_number1 .= date("m");
                                $next_number1 .= str_pad($next_number, 4, '0', STR_PAD_LEFT);
                          $pur_order_number = (isset($pur_order) ? str_pad($pur_order->number,5,'0',STR_PAD_LEFT) : str_pad($next_number1,5,'0',STR_PAD_LEFT));
                          
                          $number = (isset($pur_order) ? $pur_order->number : $next_number1);
                          if($CI->input->get('return')!=1 ){
                              echo form_hidden('number',max_number_pur_order());
                             } else{
                                 echo form_hidden('number',max_number_pur_order_r());
                             }
                             ?>
                             
                          <label for="pur_order_number"><?php echo "Id"; ?></label>
                          <div class="input-group" id="discount-total"><div class="input-group-addon">
                                <div class="dropdown">
                                   <span class="discount-type-selected">
                                    <?php if($CI->input->get('return')!=1){ echo html_entity_decode($prefix);}
                                    else{ echo html_entity_decode("VCM");}?>
                                   </span>
                                </div>
                             </div>
                             ...
                             <?php } else{?>
                              <input type="text"  class="form-control" name="pur_order_number" value="<?php  echo isset($pur_order) ? filter_var($pur_order->pur_order_number, FILTER_SANITIZE_NUMBER_INT) : html_entity_decode(max_number_pur_order_r() + 1); ?>" >
                             <?php
                             }
                             ?>
                        ...
                        <?php if(get_purchase_option('purchase_order_setting') == 0 && $CI->input->get('return')!=1 ){ ?>
                          <div class="col-md-5 form-group">
                            <label for="estimate"><small class="req text-danger">* </small><?php echo _l('PO'); ?></label>
...
<script>
            $('#pur_order_name').change(function () {
                
                var orderName = $(this).val();
                $.ajax({
                  type: 'POST',
                  url: '<?php echo base_url("purchase/check_order_name"); ?>', // replace with your controller method URL
                  data: { pur_order_name: orderName },
                  success: function(response) {
                    if (response === 'true') {
                      alert('Order name already exists!');
                      $('#pur_order_name').val(''); // clear the input field
                    } else {
                      // order name does not exist, do nothing
                    }
                  }
                });
            });
