...
function get_vendor_company_name($userid, $prevent_empty_company = false)
{
    ...
    $client = $CI->db->select('company, address,phonenumber,vat')
    ->where('userid', $_userid)
    ->from(db_prefix() . 'pur_vendor')
    ->get()
    ->row();
    if ($client) {
        return $client->company.'<br>'.$client->address.'<br>Ph: '.$client->phonenumber.'<br>Vat: '.$client->vat ;
    }
...
function format_pur_estimate_number($id)
{
    ...
    $number = sales_number_format($estimate->number, $estimate->number_format, "PO-", $estimate->date);
...
function max_number_pur_order(){
    $CI           = & get_instance();
    $max = $CI->db->query('select MAX(number) as max from '.db_prefix().'pur_orders where returns=0')->row();
    return $max->max;
}
function max_number_pur_order_r(){
    $CI           = & get_instance();
    $max = $CI->db->query('select MAX(number) as max from '.db_prefix().'pur_orders where returns=1')->row();
    return $max->max;
}
