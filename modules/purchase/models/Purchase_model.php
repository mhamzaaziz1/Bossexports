...
{   
    ...
    public function check_order_name($order_name)
  {
    ...
    $query = $this->db->get('tblpur_orders');
    if ($query->num_rows() > 0) {
      return true; // order name exists
    } else {
      return false; // order name does not exist
    }
  }
    ...
    public function get_items(){
       return $this->db->query('select id as id, CONCAT(commodity_code," - " ,description) as label from '.db_prefix().'items where isactive=1')->result_array();
    }
     ...
     * @return     <array>  The pur request detail in estimate.
     */
    public function get_so_request_detail_in_estimate($pur_request){
        $this->db->where('rel_id',$pur_request);
        $this->db->where('rel_type',"estimate");
        $this->db->select(db_prefix() . 'items.id as item_code');
        $this->db->select('qty as quantity');
        $this->db->join(db_prefix() . 'items', db_prefix() . 'items.description = ' . db_prefix() . 'itemable.description', 'left');
    ...
    public function get_pur_order_unpaid($vid){
        ...
        // $this->db->join(db_prefix() . 'pur_order_payment', '' . db_prefix() . 'pur_order_payment.pur_order = ' . db_prefix() . 'pur_orders.id', 'left');
        $this->db->where('vendor',$vid);
        if($vid==60){
            $this->db->where('order_date >=', '2024-08-01');
        }
     ...
     * { change status pur order }
     *
     * @param      <type>   $pur_order_name  Supplier ID
     * @param      <type>   $id      The identifier
     *
     * @return     boolean  ( description_of_the_return_value )
     */
    public function supplier_invoice($pur_order_name,$id){
        $this->db->where('id',$id);
        $this->db->update(db_prefix().'pur_orders',['pur_order_name' => $pur_order_name]);
        if($this->db->affected_rows() > 0){
            return true;
        }
        return false;
    ...
    public function add_pur_order($data){
        ...
        // var_dump($data['pur_order_number']); die;
        ...
        if ($insert_id) {
            ...
            foreach($es_detail as $key => $rqd){
                ...
                if($data['return']==1){
                $es_detail[$key]['quantity'] = $es_detail[$key]['quantity']*-1;
                }
                $total['total'] += $rqd['total_money'];
                $total['total_tax'] += ($rqd['total']-$rqd['into_money']);
                $total['subtotal'] += $rqd['into_money'];
    ...
    public function get_pur_request_pdf_html($pur_request_id){
        ...
        $pur_request = $this->get_purchase_order($pur_request_id);
      ...
      $html .=  '<table class="table table-hover table-bordered border_table" border="1px" style="font-size:14px">
        <tbody>
          <tr class="border_tr">
            <td class="border_td">'._l('SKU').'</td>
            <td class="border_td">'._l('Description').'</td>
      ...
      foreach($pur_request_detail as $row){
        ...
        $html .= '<tr class="border_tr">
            ...
            <td class="border_td_left">'.$items->description.'</td>
      ...
      if(1){
        $html .= '<tr id="subtotal">
                    <td width="33%"></td>
                     <td>'._l('subtotal').' </td>
                     <td class="subtotal">
                        '.app_format_money($pur_request->subtotal,'').'
                     </td>
                  </tr>
                  <tr id="subtotal">
                  <td width="33%"></td>
                     <td>'._l('discount(%)').'(%)'.'</td>
                     <td class="subtotal">
                        '.app_format_money($pur_request->discount_percent,'').' %'.'
                     </td>
                  </tr>
                  <tr id="subtotal">
                  <td width="33%"></td>
                     ...
                     <td class="subtotal">
                        '.app_format_money($pur_request->discount_total, '').'
                     </td>
                  </tr>
                  <tr id="subtotal">
                  <td width="33%"></td>
                     <td>Tax</td>
                     <td class="subtotal">
                        '.app_format_money($pur_request->total_tax, '').'
                     </td>
                  </tr>';
      }
      $html .= '<tr id="subtotal">
                 ...
                 <td>'. _l('total').'</td>
                 <td class="subtotal">
                    '. app_format_money($pur_request->total, '').'
                 </td>
              </tr>';
    ...
    $html = '<table >
      ...
      foreach($pur_request_detail as $row){
        ...
        $units = $this->get_units_by_id($row['unit_id']);
        $html .= '<tr class="border_tr">
            <td class="border_td_left">'.$items->commodity_code.'</td>
            <td class="border_td_left">'.$items->description.'</td>
     ...
     */
    public function add_payment($data, $pur_order,$bulk=0){
        $data['date'] = to_sql_date($data['date']);
    ...
    public function get_purorder_pdf_html($pur_order_id){
        ...
        if($pur_order->returns==1){  
            $info_right_column ='<span style="font-weight:bold;font-size:24px;" align="right">                                          '.mb_strtoupper(_l('Vendor')).' CLAIM</span><br/>';
            }else{
            $info_right_column ='<span style="font-weight:bold;font-size:24px;" align="right">                                           '.mb_strtoupper(_l('purchase_order')).'</span><br/>';
            } 
        if ($pur_order->returns!=0){
