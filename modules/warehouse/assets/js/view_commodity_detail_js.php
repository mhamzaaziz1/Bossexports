...
var ProposalServerParams = {
  ...
  var   table_warehouse_PO = $('table.table-table_warehouse_PO');
   var  _table_api = initDataTable(table_warehouse_PO, admin_url+'warehouse/table_warehouse_PO', [], [], ProposalServerParams, [0, 'desc']);

   $('.table-table_warehouse_PO').DataTable().columns([0]).visible(false, false);
...
var ProposalServerParams = {
  ...
  var   table_warehouse_so = $('table.table-table_warehouse_so');
   var  _table_api = initDataTable(table_warehouse_so, admin_url+'warehouse/table_warehouse_so', [], [], ProposalServerParams, [0, 'desc']);

   $('.table-table_warehouse_so').DataTable().columns([0]).visible(false, false);

    $.each(ProposalServerParams, function(i, obj) {
        $('select' + obj).on('change', function() {  
            table_warehouse_so.DataTable().ajax.reload()
                .columns.adjust()
                .responsive.recalc();
        });
    });
