...
class Warehouse_model extends App_Model {
	 ...
	 * @return array or object
	 */
	public function get_inventory_minmax($id = false) {
			$this->db->where('commodity_id', $id);
			return $this->db->get(db_prefix() . 'inventory_commodity_min')->row();
	}
	...
	public function get_stock_import_pdf_html($goods_receipt_id) {
		...
		<tbody>
		<tr>
		<td class="text-left">'.pdf_logo_url().'</td>
		<td class="text_right_weight "><h3>' . mb_strtoupper(_l('receiving')) . ' VOUCHER</h3></td>
		</tr>

		<tr>
		<td></td>
		<td class="text_right_weight">Voucher No# '.$goods_receipt->goods_receipt_code.'</td>
		</tr>
		</tbody>
		</table>
		<br><br><br>
		...
		//organization_info
		$organization_info = '<div  class="bill_to_color" style="font-size:14px;">';
		...
		if (0){
			...
			$bill_to .= '<div class="bill_to_color" style="font-size:14px;">';
		...
		foreach ($goods_receipt_detail as $receipt_key => $receipt_value) {
			...
			$html .= '<tr>';
			$html .= '<td class="td_style_r_ep_c"><b>' . $commodity_code . '_'. $commodity_name.'</b></td>
	...
	public function get_stock_export_pdf_html($goods_delivery_id) {
		...
		<tr>
		<td class="text_right">Delivery Voucher# '.$goods_delivery->goods_delivery_code.'</td>
		</tr>
		</tbody>
		</table>
		<br><br><br>
		';
	     ...
	     //organization_info
		$organization_info = '<div style="font-size: 14px;">';
		$organization_info .= format_organization_info();
		$organization_info .= '</div>';
	    ...
	    //invoice_data_date
		$invoice_date = '<b>' . _l('invoice_data_date') . ' ' . _d($goods_delivery->date_add) . '</b>';

		$html .= '<table class="table" style="font-size: 15px;">
		<tbody>
		<tr>
		<td rowspan="2" width="50%" class="text-left">'.$organization_info.'</td>
		<td rowspan="2" width="50%" class="text_right">'.$bill_to.'</td>
		...
		if(get_warehouse_option('goods_delivery_pdf_display') == 1){
			...
			$html1 .= '<table class="table" style="font-size: 14px;">
			<tbody>
			<tr>
			<td ></td>
			<td ></td>
	...
	public function get_stock_summary_report($data) {
		...
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			...
			$stock_opening_quatity = ABS($closing_quatity -(float)$import_period_quatity + (float)$export_period_quantity);
	...
	public function get_stock_summary_report_view($data) {
		...
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			...
			//$closing_quatity = $stock_opening_quatity + $import_period_quatity - $export_period_quatity;
			// before get from fomular: $closing_amount = ($stock_opening_amount + $import_period_amount - $export_period_amount) after change below
			$closing_quatity = $this->warehouse_model->get_quantity_inventory($data['warehouse_id'], $commodity_list['commodity_id'])->inventory_number;
			if(!isset($closing_quatity)){
			    $closing_quatity=0;
			}
// 			var_dump($closing_quatity);die;
			$closing_amount = $closing_quatity*(float)$purchase_price;
...
public function delete_goods_receipt($id) {
    ...
    public function add_goods_delivery_from_invoice($data_insert, $invoice_id ='')
    {
    	...
    	if (isset($insert_id)) {
    		...
    		foreach ($data_insert['goods_delivery_detail'] as $delivery_detail_key => $delivery_detail_value) {
                ...
                $delivery_detail_value['warehouse_id'] = '1';
    ...
    {	
    	...
    	}elseif(isset($data['commodity_code']) && $data['commodity_code'] != ''){
    	//check insert
    		$this->db->where('commodity_code', $data['commodity_code']);
    		$items = $this->db->get(db_prefix() . 'items')->row();
    		if($items){
