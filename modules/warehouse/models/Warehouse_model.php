...
class Warehouse_model extends App_Model {
    ...
    public function auto_create_goods_delivery_with_credit($invoice_id, $invoice_update='')
    {
    	...
    	$this->db->where('id', $invoice_id);
    	$invoice_value = $this->db->get(db_prefix().'creditnotes')->row();
    		...
    		if(count($arr_itemable) > 0){
    			foreach ($arr_itemable as $key => $value) {
    				...
    				if($commodity_code != 0){
    					...
    					$arr_item_insert[$index]['quantities'] = $value['qty'] *-1;
   ...
   public function update_po_detail_quantity($po_id, $goods_receipt_detail)
    ...
    public function create_goods_receipt_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $lot_number = '', $date_manufacture = '', $expiry_date = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $tax_money = '', $goods_money = '', $note = '', $item_key = '', $sub_total = '', $tax_name = '', $tax_id = '', $is_edit = false, $serial_number = '') {
		...
		$row .= '<td class="weight_volume">' . 
		render_input($name . '[weight]', '', $weight, 'text', ['placeholder' => _l('weight'), 'readonly' => true], [], 'no-margin', 'input-transparent text-right') . 
		render_input($name . '[volume]', '', $volume, 'text', ['placeholder' => _l('volume'), 'readonly' => true], [], 'no-margin', 'input-transparent text-right').
		'</td>';
	...
	public function  order_return_get_inventory_receipt()
	{
		...
		$own_goods_receipt = '';
		if (staff_cant('wh_stock_import', '', 'view')) {
	...
	public function revert_serial_number($commodity_id, $warehouse_id, $inventory_manage_id, $str_serial_number)
	{
		if(new_strlen($str_serial_number) > 0){
			...
			$this->db->limit(abs((int)$inventory_manage_id));
	...
	public function get_serial_number_for_delivery_note($commodity_id, $warehouse_id, $inventory_manage_id, $quantity, $serial_number, $goods_delivery_detail_id, $commodity_name)
	{
		...
		// Check if it's a return (credit note)
		$is_return = ((float)$quantity < 0);
		$abs_quantity = abs((int)$quantity);
		$is_used_status = $is_return ? 'yes' : 'no';
		$new_is_used_status = $is_return ? 'no' : 'yes';

		//check serial number is used
		$this->db->where('commodity_id', $commodity_id);
		$this->db->where('warehouse_id', $warehouse_id);
		$this->db->where('serial_number', $serial_number);
		$this->db->where('is_used', $is_used_status);
		$this->db->order_by('id', 'asc');
		$this->db->limit($abs_quantity);
		$inventory_serial_numbers = $this->db->get(db_prefix().'wh_inventory_serial_numbers')->row();

		if($inventory_serial_numbers && new_strlen($serial_number) > 0){
			$this->db->where('id', $inventory_serial_numbers->id);
			$this->db->update(db_prefix().'wh_inventory_serial_numbers', ['is_used' => $new_is_used_status]);
	...
	public function get_list_temporaty_serial_numbers($commodity_id, $warehouse_id, $quantity = '', $where = [] )
	{
		...
		if(is_numeric($quantity)){
			$this->db->limit(abs((int)$quantity));
		}
		$inventory_serial_numbers = $this->db->get(db_prefix().'wh_inventory_serial_numbers')->result_array();
	...
	/**
	 * Get commodity monthly data for charts
	 * @param  integer $commodity_id
	 * @param  string  $type
	 * @param  integer $year
	 * @return array
	 */
	public function get_commodity_monthly_data($commodity_id, $type, $year)
	{
		$final_result = [];
		$item = $this->db->get_where(db_prefix() . 'items', ['id' => $commodity_id])->row();

		if (!$item) {
			return ['amounts' => array_fill(0, 12, 0), 'trend' => array_fill(0, 12, 0)];
		}

		$amounts = array_fill(0, 12, 0);

		if ($type == 'sales') {
			$this->db->select('MONTH(i.date) as month, SUM(it.qty * it.rate) as total_amount');
			...
			$this->db->group_by('MONTH(i.date)');
			$result = $this->db->get()->result_array();

			foreach ($result as $row) {
				$amounts[$row['month'] - 1] = (float)$row['total_amount'];
			}
		} elseif ($type == 'purchases') {
			// Check for purchase module
			if ($this->db->table_exists(db_prefix() . 'pur_orders')) {
				$this->db->select('MONTH(po.order_date) as month, SUM(it.qty * it.rate) as total_amount');
				$this->db->from(db_prefix() . 'itemable as it');
				...
				$this->db->where('it.description', $item->description);
				$this->db->where('YEAR(po.order_date)', $year);
				$this->db->group_by('MONTH(po.order_date)');
				$result = $this->db->get()->result_array();
		...
		return ['amounts' => $amounts, 'trend' => $trend];
	}
