...
$rResult = $result['rResult'];
    ...
    if($aRow[db_prefix().'goods_transaction_detail.status'] == 1){
         ...
         $CI =& get_instance();
         $row[] =get_vendor_company_name($CI->db->query('select tblpur_orders.vendor as pur from tblgoods_receipt INNER JOIN tblpur_orders ON tblgoods_receipt.pr_order_id=tblpur_orders.id  where tblgoods_receipt.id ='.$aRow['goods_receipt_id'])->row()->pur);
        //  get_vendor_company_name($pur_request->vendor_id);
         ...
         $CI =& get_instance();
         $row[] =$CI->db->query('select tblclients.company as pur from tblgoods_delivery INNER JOIN tblclients ON tblgoods_delivery.customer_code=tblclients.userid  where tblgoods_delivery.id ='.$aRow['goods_receipt_id'])->row()->pur;
        //  get_vendor_company_name($pur_request->vendor_id);
    ...
    //  var_dump($aRow); die;
     ...
     $row[] = _d($aRow['tblgoods_transaction_detail.date_add']); 
