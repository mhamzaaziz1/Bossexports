...
return (new App_table('table_warehouse_history'))
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            db_prefix().'goods_transaction_detail.commodity_id',
        ...
        if(isset($commodity_ft)){
            if(!is_array($commodity_ft)){
                $where_commodity_ft = ' AND '.db_prefix().'goods_transaction_detail.commodity_id = "'.$commodity_ft.'"';
                ...
                foreach ($commodity_ft as $commodity_id) {
                    if($commodity_id != '')
                    {
                        if($where_commodity_ft == ''){
                            $where_commodity_ft .= ' AND ('.db_prefix().'goods_transaction_detail.commodity_id = "'.$commodity_id.'"';
                        }else{
                            $where_commodity_ft .= ' or '.db_prefix().'goods_transaction_detail.commodity_id = "'.$commodity_id.'"';
                        }
                    }
                }
                if($where_commodity_ft != '')
        ...
        if(isset($status_ft)){
            ...
            foreach ($status_ft as $status_id) {
                ...
                {
                    if($where_status_ft == ''){
                        $where_status_ft .= ' AND ('.db_prefix().'goods_transaction_detail.status = "'.$status_id.'"';
                    }else{
                        $where_status_ft .= ' or '.db_prefix().'goods_transaction_detail.status = "'.$status_id.'"';
                    }
                }
            }
            if($where_status_ft != '')
        ...
        $start_date = null;
        $end_date = null;
        if($CI->input->post('validity_start_date')){
            $start_date = to_sql_date($CI->input->post('validity_start_date'));
        }
        if($CI->input->post('validity_end_date')){
            $end_date = to_sql_date($CI->input->post('validity_end_date'));
        }
        ...
        $result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            ...
            db_prefix().'goods_receipt.date_add as 1_date_add',
            db_prefix().'goods_delivery.date_add as 2_date_add',
            db_prefix().'internal_delivery_note.date_add as 4_date_add',
            db_prefix().'wh_loss_adjustment.date_create as 3_date_add'
        ]);
        ...
        foreach ($rResult as $aRow) {
            ...
            if($aRow[db_prefix().'goods_transaction_detail.status'] == 1){
                 ...
                 if($value != ''){
                    $form_code = '<a href="' . admin_url('warehouse/manage_purchase/' . $aRow['goods_receipt_id']) . '" >'. $value.'</a>';
                 }
                 ...
                 if($value != ''){
                    $form_code = '<a href="' . admin_url('warehouse/manage_delivery/' . $aRow['goods_receipt_id']) . '" >'. $value.'</a>';
                 }
                 ...
                 if($value != ''){
                    $form_code = '<a href="' . admin_url('warehouse/manage_internal_delivery/' . $aRow['goods_receipt_id']) . '" >'. $value.'</a>';
                 }
            }else{
                 //3 lost adjustment
                 $value = "LA#".$aRow['goods_receipt_id'];
                 if($value != ''){
                    $form_code = '<a href="' . admin_url('warehouse/view_lost_adjustment/' . $aRow['goods_receipt_id']) . '" >'. $value.'</a>';
                 }
            }
            $row[] = $form_code;

             $row[] = get_commodity_name($aRow['commodity_id']) != null ? get_commodity_name($aRow['commodity_id'])->commodity_code : '';
             // Removed description column to match headers
            ...
            // Lot Number / Quantity Sold
             ...
             $row[] = $lot_number . ' / ' . $aRow['quantity']; // Combined
            ...
            // Expiry Date
             ...
             // Serial Number (Placeholder as per headers)
             $row[] = ''; 
