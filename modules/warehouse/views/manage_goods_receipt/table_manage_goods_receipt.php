...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'supplier_name'){
            ...
            // }else if($aRow['supplier_name']!=""){
            //     $_data = $aRow['supplier_name'];
            // }else{
            // $this->ci->db->select('tblpur_vendor.company as vendor');
            // $this->ci->db->from(db_prefix() . 'pur_orders');
            // $this->ci->db->join(db_prefix() . 'pur_vendor'  , 'tblpur_vendor.userid = tblpur_orders.vendor' );
            // $this->ci->db->where(db_prefix() . 'pur_orders.id', $aRow['pr_order_id']);
            // $e=$this->ci->db->get()->result();
            //     $_data = $e[0]->vendor;
            // }
            $CI =& get_instance();
            $_data = get_vendor_company_name($CI->db->query('select tblpur_orders.vendor as pur from tblpur_orders INNER JOIN tblgoods_receipt ON tblpur_orders.id = tblgoods_receipt.pr_order_id  where tblgoods_receipt.pr_order_id ='.$aRow['pr_order_id'])->row()->pur);
