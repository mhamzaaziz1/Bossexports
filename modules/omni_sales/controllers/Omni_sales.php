...
class Omni_sales extends AdminController
{
			...
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					...
					$select = [
						...
						db_prefix() . 'sales_channel_detailt.prices'         
					];
					if($channel == 'pos'){
						$select[] = db_prefix() . 'sales_channel_detailt.department';
					}
		...
		public function order_list($id = ''){
			if (staff_cant('omni_order_list', '', 'view') && staff_cant('omni_order_list', '', 'view_own') && !is_admin()) {
	...
	public function get_store_woo(){
		...
		public function omi_sales_delete_bulk_action()
		{
			...
			if ($this->input->post()) {
				...
				switch ($rel_type) {
					case 'omni_sales':
					if (staff_cant('omni_sales', '', 'delete') && !is_admin()) {
	...
	public function delete_order($id){
		if (staff_cant('omni_order_list', '', 'delete') && !is_admin()) {
			access_denied('omni_order_list');
		}
	...
	public function customer_report()
	{
		...
		if(staff_cant('omni_sales', '', 'view')  &&  !is_admin()) {
			access_denied('omni_sales');
		}
 ...
 /**
	...
	*/
	public function table_manage_import_customer_reports() {
		...
		App_table::find(module_views_path('omni_sales', 'import_customer_report/table_customer_report'));
	}
	...
	public function customer_report_delete_bulk_action()
	{
		...
		public function manage_customer_report() {
			if (staff_cant('omni_sales', '', 'edit') && !is_admin() && staff_cant('omni_sales', '', 'create')) {
				access_denied('omni_sales');
			}
		...
		public function table_manage_create_customer_reports() {
			...
			App_table::find(module_views_path('omni_sales', 'import_customer_report/table_create_customer_report'));
		}
		...
		public function table_view_customer_report_detail() {
			...
			App_table::find(module_views_path('omni_sales', 'import_customer_report/table_view_customer_report_detail'));
		}
	 ...
	 */
		public function table_manage_import_mpesatransc() {
			...
			App_table::find(module_views_path('omni_sales', 'import_customer_report/table_manage_import_mpesatransc'));
		}
	 ...
	 public function order_manual($order_id = ''){
	 	...
	 	if($this->input->post()){
	 		...
	 		if($data['id'] == ''){
	 			...
	 			if (staff_cant('omni_order_list', '', 'create') && !is_admin()) {
					access_denied('omni_order_list');
				}
	 			...
	 			if (staff_cant('omni_order_list', '', 'edit') && !is_admin()) {
					access_denied('omni_order_list');
				}
...
public function create_purchase_request(){
	...
	*/
	...
	{
		App_table::find(module_views_path('omni_sales', 'order_returns/table_order_return'));
	}
	 ...
	 */
	 ...
	 public function table_refund(){
		App_table::find(module_views_path('omni_sales', 'table/table_refund'));
	 }
