...
class Omni_sales_client extends ClientsController
{
   ...
   */
     ...
     * @return view       
     */
      public function index($page='',$id = '0',$key = '0',$warehouse = 0){  
        //   $this->add_product();
          $data['styles'] = $this->omni_sales_model->get_style_add_commodity();
          $data['ofset'] = 24;
          $data['title'] = _l('sales');
          $data['group_product'] = $this->omni_sales_model->get_group_product();        
          $data['group_id'] = $id;
          $data_product = $this->omni_sales_model->get_list_product_by_group(2,$id,$key,($page-1)*$data['ofset'],$data['ofset'],$this->input->get('type'),$this->input->get('style_id'));
          $data['product'] = [];
          $date = date('Y-m-d');
        //   var_dump($data_product['list_product']);
		  ...
		  $data['commodity_types'] = $this->omni_sales_model->get_commodity_type_add_commodity();
          $this->data($data);
     ...
     public function get_product_by_group($page='',$id = '',$key = ''){  
      ...
      public function search_product($group_id){
          if($this->input->post()){
            ...
            redirect(site_url('omni_sales/omni_sales_client/index/1/'.$group_id.'/'.$data['keyword'].'?type='.$data['commodity_type'].'&style_id='.$data['style_id']));                    
          }
      }
      /**
       * get stock 
