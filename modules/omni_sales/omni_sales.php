...
hooks()->add_action('admin_init', 'omni_sales_add_client_tab');

/**
 * Init client tab via app_tabs
 */
function omni_sales_add_client_tab()
{
    $CI = &get_instance();
    if (has_permission('omni_sales', '', 'view') || is_customer_admin(get_client_user_id())) {
        $CI->app_tabs->add_customer_profile_tab('performance_matrix', [
            'name'     => _l('Performance Matrixes'),
            'icon'     => 'fa fa-bar-chart',
            'view'     => 'omni_sales/client/performance_matrix',
            'position' => 19, // Before Statement (20)
        ]);
    }
}

/**
 * Helper to get data for the view since app_tabs loads the view directly
 * We need to inject the data into the global data or use a filter/hook if possible.
 * However, CodeIgniter views loaded this way might share $data.
 * But we need to calculate the stats.
 * Best approach: Use a view composer or load the data inside the view itself or use the 'client_filtered_visible_tabs' hook to inject data? No.
 * simpler: In the view file, we can load the model and fetch data if it's not passed.
 * Or, use the 'before_client_profile_loaded' (if exists) or similar.
 * 
 * Actually, let's check how other module tabs get data.
 * Usually controllers/admin/Clients.php loads the view and passes $data.
 * The $data includes 'client'.
 * So in the view, we can:
 * $CI = &get_instance();
 * $CI->load->model('omni_sales/omni_sales_model');
 * $performance = $CI->omni_sales_model->get_client_performance_stats($client->userid);
 */