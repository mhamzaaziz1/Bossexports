...
class Omni_sales_model extends App_Model
{
  ...
  public function add_product($data){
    ...
    if(/*$data['group_product_id'] && empty($data['product_id']) &&*/ 0){
    ...
    //   var_dump($value['rate']);die;

      $data_add['sales_channel_id'] = $data['sales_channel_id'];
      $data_add['group_product_id'] = $value['group_id'];
      $data_add['id'] = $value['id'];
      $data_add['prices'] = $value['rate'];
      $data_saved = $this->get_product_channel($value['id'],$data['sales_channel_id']);
   ...
   * @return  array $result             
   */
   public function get_list_product_by_group($id_chanel, $id_group = '0',$key = '',$limit = 0, $ofset = 1, $commodity_type=" ", $style_id=""){
       if($commodity_type!="" ||$commodity_type!=NULL){
           $c=' and commodity_type='.$commodity_type;
       }
       if($style_id!="" ||$style_id!=NULL){
           $c.=' and style_id='.$style_id;
       }
    ...
    if($key != ''){
      if($id_group!='0'){
        ...
        $select_list_product = 'select  id, description, long_description, rate, sku_code, tax, group_id from '.db_prefix().'items where description like \'%'.$key.'%\' and group_id = '.$id_group.'  '.$c .'  limit '.$limit.','.$ofset;
        $result = [
        ...
        $select_list_product = 'select  id, description, long_description, rate, sku_code, tax, group_id from '.db_prefix().'items where  description like \'%'.$key.'%\'  '. $c.'  limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
      ...
      if($id_group!='0'){
        ...
        $select_list_product = 'select  id, description, long_description, rate, sku_code, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.') and group_id = '.$id_group.' '.$c .'  limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
        ...
        $select_list_product = 'select  id, description, long_description, rate, sku_code, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.') '.$c .'  limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
 ...
 */
  ...
  public function add_inv_when_order($orderid, $status = '') {
    ...
    if($cart){
      ...
      $data["subject"]='Customer Request';
      $data["subject"]= "Sales Quotation";
          $data["rel_type"]= "customer";
          $data["rel_id"]=  $data["clientid"];
          $data["open_till"]=  $data["duedate"];
          $data["discount_type"]=  "";
          $data["tags"]=  "";
          $data["allow_comments"]= "on";
         $data["status"]=  "6";
          $data["assigned"]= "";
          $data["proposal_to"]="Admin";
          ...
          unset($data["show_shipping_on_invoice"]);
