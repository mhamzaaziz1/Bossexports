...
/**
 ...
 /**
     ...
     * @return array
     */
    public function get_client_performance_stats($clientid, $period = 'all_time')
    {
        $this->load->model('invoices_model');
        $this->load->model('credit_notes_model');
        ...
        // 1. Revenue & Value Metrics
        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_PAID);
        $apply_filter($this->db, 'date'); // Apply Filter
        $clv = $this->db->get(db_prefix() . 'invoices')->row()->total;
        $clv = $clv ? $clv : 0;

        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_PAID);
        $total_paid_invoices = $this->db->count_all_results(db_prefix() . 'invoices');

        $aov = $total_paid_invoices > 0 ? $clv / $total_paid_invoices : 0;

        // Revenue Growth (Current Year vs Last Year)
        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_PAID);
        $this->db->where('YEAR(date)', date('Y'));
        $revenue_this_year = $this->db->get(db_prefix() . 'invoices')->row()->total;

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_PAID);
        $this->db->where('YEAR(date)', date('Y') - 1);
        $revenue_last_year = $this->db->get(db_prefix() . 'invoices')->row()->total;

        $revenue_growth = 0;
        if ($revenue_last_year > 0) {
            $revenue_growth = (($revenue_this_year - $revenue_last_year) / $revenue_last_year) * 100;
        }

        // Discount Dependency
        $this->db->where('clientid', $clientid);
        $this->db->where('discount_total >', 0);
        $apply_filter($this->db, 'date');
        $discounted_invoices = $this->db->count_all_results(db_prefix() . 'invoices');
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $total_invoices_all = $this->db->count_all_results(db_prefix() . 'invoices');
        $discount_dependency = $total_invoices_all > 0 ? ($discounted_invoices / $total_invoices_all) * 100 : 0;
        
        // Net Revenue (Revenue - Expenses)
        $this->db->select_sum('amount');
        $this->db->where('clientid', $clientid);
        $this->db->where('billable', 1); // Only count billable expenses? Or all? Usually Net Revenue = Income - All Direct Expenses
        $apply_filter($this->db, 'date');
        $total_expenses = $this->db->get(db_prefix() . 'expenses')->row()->amount;
        $net_revenue = $clv - ($total_expenses ? $total_expenses : 0);


        // 2. Purchase Behavior Metrics
        // First Purchase Date
        $this->db->select_min('date');
        $this->db->where('clientid', $clientid);
        $first_purchase = $this->db->get(db_prefix() . 'invoices')->row()->date;

        $purchase_frequency = 0;
        if ($first_purchase) {
            $months_since_first = (strtotime(date('Y-m-d')) - strtotime($first_purchase)) / (60 * 60 * 24 * 30);
            if ($months_since_first >= 1) {
                $purchase_frequency = $total_invoices_all / $months_since_first;
            } else {
                $purchase_frequency = $total_invoices_all;
            }
        }
        ...
        $this->db->where('clientid', $clientid);
        $last_purchase = $this->db->get(db_prefix() . 'invoices')->row()->date;
        $recency_days = $last_purchase ? round((time() - strtotime($last_purchase)) / (60 * 60 * 24)) : 'N/A';
        ...
        $this->db->where('rel_type', 'invoice');
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, db_prefix().'invoices.date');
        $total_items_sold = $this->db->count_all_results(db_prefix() . 'itemable');

        $avg_basket_size = $total_invoices_all > 0 ? $total_items_sold / $total_invoices_all : 0;

        // 3. Engagement Metrics
        // CRM Interaction Score (Tickets + Notes + Reminders)
        ...
        $total_notes = $this->db->count_all_results(db_prefix() . 'notes');

        $this->db->where('rel_id', $clientid);
        $this->db->where('rel_type', 'customer');
        $total_reminders = $this->db->count_all_results(db_prefix() . 'reminders');

        $interaction_score = $total_tickets + $total_notes + $total_reminders;

         // 4. Retention & Loyalty
        // Tenure
        $this->db->select('datecreated');
        $this->db->where('userid', $clientid);
        $client_rec = $this->db->get(db_prefix().'clients')->row();
        $tenure_days = $client_rec ? round((time() - strtotime($client_rec->datecreated)) / (60 * 60 * 24)) : 0;

        // Churn Risk (Simple Heuristic)
        $churn_risk = 'Low';
        if ($recency_days !== 'N/A') {
            if ($recency_days > 180) $churn_risk = 'High';
            elseif ($recency_days > 90) $churn_risk = 'Medium';
        }

        // 5. Satisfaction & Risk
        // Complaint Frequency (Tickets / Invoices raw)
        $complaint_frequency = $total_tickets;

        // Payment Risk
        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_OVERDUE);
        $overdue_invoices = $this->db->count_all_results(db_prefix() . 'invoices');
        $payment_risk_score = $total_invoices_all > 0 ? ($overdue_invoices / $total_invoices_all) * 100 : 0;


        // 6. Predictive (Mock Logic)
        $upsell_prob = 'Low';
        if($aov > 1000 && $purchase_frequency > 1) $upsell_prob = 'High';
        elseif($aov > 500) $upsell_prob = 'Medium';

        $customer_segment = 'New';
        if($clv > 5000) $customer_segment = 'VIP';
        elseif($clv > 1000) $customer_segment = 'Loyal';
        elseif($recency_days > 90 && $clv > 0) $customer_segment = 'At-Risk';
        
        // 7. Sales Pipeline (New)
        // Proposal Conversion
        // Proposals: rel_type='customer' AND rel_id=clientid
        $this->db->where('rel_type', 'customer');
        $this->db->where('rel_id', $clientid);
        $apply_filter($this->db, 'date');
        $total_proposals = $this->db->count_all_results(db_prefix().'proposals');
        
        $this->db->where('rel_type', 'customer');
        $this->db->where('rel_id', $clientid);
        $this->db->where('status', 3); // Accepted
        $apply_filter($this->db, 'date');
        $accepted_proposals = $this->db->count_all_results(db_prefix().'proposals');
        $proposal_conversion = $total_proposals > 0 ? ($accepted_proposals / $total_proposals) * 100 : 0;

        // Estimate Conversion
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $total_estimates = $this->db->count_all_results(db_prefix().'estimates');
        
        $this->db->where('clientid', $clientid);
        $this->db->where('status', 4); // Accepted
        $apply_filter($this->db, 'date');
        $accepted_estimates = $this->db->count_all_results(db_prefix().'estimates');
        $estimate_conversion = $total_estimates > 0 ? ($accepted_estimates / $total_estimates) * 100 : 0;
        ...
        // 9. Detailed Sales Stats (Counts & Amounts)
        // Proposals
        $this->db->where('rel_type', 'customer');
        $this->db->where('rel_id', $clientid);
        $apply_filter($this->db, 'date');
        $count_proposals = $this->db->count_all_results(db_prefix().'proposals');

        $this->db->select_sum('total');
        $this->db->where('rel_type', 'customer');
        $this->db->where('rel_id', $clientid);
        $apply_filter($this->db, 'date');
        $amount_proposals = $this->db->get(db_prefix() . 'proposals')->row()->total;
        $amount_proposals = $amount_proposals ? $amount_proposals : 0;

        // Estimates
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $count_estimates = $this->db->count_all_results(db_prefix().'estimates');

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $amount_estimates = $this->db->get(db_prefix() . 'estimates')->row()->total;
        $amount_estimates = $amount_estimates ? $amount_estimates : 0;

        // Invoices (Gross Sales)
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $count_invoices = $this->db->count_all_results(db_prefix().'invoices');

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $amount_invoices = $this->db->get(db_prefix() . 'invoices')->row()->total;
        $gross_sales = $amount_invoices ? $amount_invoices : 0; // Gross Sales

        // Credit Notes (Credit Notes Value)
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $count_credit_notes = $this->db->count_all_results(db_prefix().'creditnotes');

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $apply_filter($this->db, 'date');
        $amount_credit_notes = $this->db->get(db_prefix() . 'creditnotes')->row()->total;
        $credit_notes_value = $amount_credit_notes ? $amount_credit_notes : 0;

        // Net Sales
        $net_sales = $gross_sales - $credit_notes_value;
        ...
        $this->db->where('clientid', $clientid);
        $this->db->where('YEAR(date)', date('Y'));
        $gross_this_year = $this->db->get(db_prefix() . 'invoices')->row()->total;

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('YEAR(date)', date('Y'));
        $credit_this_year = $this->db->get(db_prefix() . 'creditnotes')->row()->total;
        $net_this_year = ($gross_this_year ? $gross_this_year : 0) - ($credit_this_year ? $credit_this_year : 0);

        // Previous Year Net
        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('YEAR(date)', date('Y') - 1);
        $gross_last_year = $this->db->get(db_prefix() . 'invoices')->row()->total;

        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('YEAR(date)', date('Y') - 1);
        $credit_last_year = $this->db->get(db_prefix() . 'creditnotes')->row()->total;
        $net_last_year = ($gross_last_year ? $gross_last_year : 0) - ($credit_last_year ? $credit_last_year : 0);

        $sales_growth_percent = 0;
        if($net_last_year > 0){
             $sales_growth_percent = (($net_this_year - $net_last_year) / $net_last_year) * 100;
        }

        // Payments
        // Need join
        $this->db->select('count('.db_prefix().'invoicepaymentrecords.id) as count, sum(amount) as total');
        ...
        $apply_filter($this->db, db_prefix().'invoicepaymentrecords.date'); // Filter by payment date
        $payment_stats = $this->db->get()->row();
        $count_payments = $payment_stats ? $payment_stats->count : 0;
        $amount_payments = $payment_stats ? $payment_stats->total : 0;
        $amount_payments = $amount_payments ? $amount_payments : 0;

        // Collection Rate %
        $collection_rate = $net_sales > 0 ? ($amount_payments / $net_sales) * 100 : 0;

        // Overdue Rate %
        $this->db->select_sum('total');
        $this->db->where('clientid', $clientid);
        $this->db->where('status', Invoices_model::STATUS_OVERDUE);
        $overdue_amount = $this->db->get(db_prefix() . 'invoices')->row()->total;
        $overdue_amount = $overdue_amount ? $overdue_amount : 0;
        $overdue_rate = $net_sales > 0 ? ($overdue_amount / $net_sales) * 100 : 0;

        // Credit Impact %
        $credit_impact = $gross_sales > 0 ? ($credit_notes_value / $gross_sales) * 100 : 0;

        // Items Stats & Product Profitability
        // Fetch all invoice items for this client to calculate Gross Product Revenue, Cost, Profit
        $this->db->select('b.description, b.qty, b.rate, i.purchase_price');
        $this->db->from(db_prefix() . 'itemable as b');
        $this->db->join(db_prefix() . 'invoices', 'tblinvoices.id = b.rel_id');
        $this->db->join(db_prefix() . 'items as i', 'i.description = b.description', 'left'); // Attempt to link to master item for cost
        $this->db->where('b.rel_type', 'invoice');
        $this->db->where('tblinvoices.clientid', $clientid);
        $apply_filter($this->db, 'tblinvoices.date');
        $invoice_items = $this->db->get()->result();

        $gross_product_revenue = 0;
        $gross_product_profit = 0;
        $net_units_sold = 0;
        $total_cost = 0;

        foreach($invoice_items as $item){
            $gross_product_revenue += $item->qty * $item->rate;
            $cost = isset($item->purchase_price) ? $item->purchase_price : 0;
            $gross_product_profit += ($item->rate - $cost) * $item->qty;
            $net_units_sold += $item->qty;
        ...
        $this->db->select('b.description, b.qty, b.rate, i.purchase_price');
        $this->db->from(db_prefix() . 'itemable as b');
        $this->db->join(db_prefix() . 'creditnotes', 'tblcreditnotes.id = b.rel_id');
        $this->db->join(db_prefix() . 'items as i', 'i.description = b.description', 'left');
        $this->db->where('b.rel_type', 'credit_note'); // Check rel_type for credit notes
        $this->db->where('tblcreditnotes.clientid', $clientid);
        $apply_filter($this->db, 'tblcreditnotes.date');
        $credit_items = $this->db->get()->result();

        $product_credit_value = 0;
        $credit_profit_impact = 0;
        $credited_quantity = 0;

        foreach($credit_items as $item){
            $product_credit_value += $item->qty * $item->rate;
            $cost = isset($item->purchase_price) ? $item->purchase_price : 0;
            $credit_profit_impact += ($item->rate - $cost) * $item->qty;
            $credited_quantity += $item->qty;
        }

        $net_product_revenue = $gross_product_revenue - $product_credit_value;
        $net_product_profit = $gross_product_profit - $credit_profit_impact;
        $net_units_sold = $net_units_sold - $credited_quantity;
        $net_margin_percent = $net_product_revenue > 0 ? ($net_product_profit / $net_product_revenue) * 100 : 0;
        $product_credit_rate = $gross_product_revenue > 0 ? ($product_credit_value / $gross_product_revenue) * 100 : 0;
        
        // Stock Turnover (Net Units Sold / Average Inventory) - Approximated as we don't have historical inventory
        // Skipping Stock Turnover for Client View as it's hard to define "Average Inventory" for a client.



        // Restore Items Stats (Count & Unique)
        $total_items_invoiced = 0;
        $unique_items_arr = [];
        if(isset($invoice_items) && is_array($invoice_items)){
            foreach($invoice_items as $itm){
                $total_items_invoiced += $itm->qty;
                $unique_items_arr[$itm->description] = 1;
            }
        }
        $total_unique_items = count($unique_items_arr);

        // 10. Averages (Corrected for Net where applicable)
        $avg_proposal_val = $count_proposals > 0 ? $amount_proposals / $count_proposals : 0;
        $avg_estimate_val = $count_estimates > 0 ? $amount_estimates / $count_estimates : 0;
        $avg_invoice_val = $count_invoices > 0 ? $amount_invoices / $count_invoices : 0; // Gross AOV
        $avg_order_value_net = $count_invoices > 0 ? $net_sales / $count_invoices : 0; // Avg Order Value (Net)

        // Customer Return Rate %
        // Customer Return Rate %
        $customer_return_rate = $gross_sales > 0 ? ($credit_notes_value / $gross_sales) * 100 : 0;
        
        // CLV Net
        // Average Annual Net Revenue * Tenure (Years)
        $avg_annual_net_revenue = $tenure_days > 365 ? ($net_sales / ($tenure_days/365)) : $net_sales;
        $clv_net = $avg_annual_net_revenue * ($tenure_days > 0 ? $tenure_days/365 : 1);
        if($clv_net == 0) $clv_net = $net_sales; // Fallback


        // 6. Credit Note Intelligence Metrics
        // Credit Frequency %
        $credit_frequency_percent = $count_invoices > 0 ? ($count_credit_notes / $count_invoices) * 100 : 0;
        
        // Avg Credit Value
        $avg_credit_value = $count_credit_notes > 0 ? $credit_notes_value / $count_credit_notes : 0;


        // 7. Composite Scores
        // Normalizing values (simple 0-1 scaling for score calculation)
        // Scores are subjective 0-100?
        // Using provided formulas:
        // Product Performance Score (0.3 × Sales) + (0.25 × Margin) ... Sales need to be normalized. 
        // Let's assume standardized scores out of 100.
        // Simplified Logic for Display:
        $prod_perf_score = min(100, ($net_product_revenue / 1000) * 0.3 + $net_margin_percent * 0.25); // Mock normalization
        $cust_perf_score = min(100, ($net_sales / 1000) * 0.3 + $purchase_frequency * 20 * 0.25 + $interaction_score * 0.2); // Mock
        $sales_health_score = $sales_growth_percent - $credit_impact - $overdue_rate;


        return [
            'revenue' => [
                'clv' => $clv, // Keeping old for backward comp if needed, otherwise replace
                'clv_net' => $clv_net,
                'aov' => $aov,
                'aov_net' => $avg_order_value_net,
                'total_revenue' => $gross_sales,
                'net_sales' => $net_sales,
                'sales_growth' => $sales_growth_percent,
                'collection_rate' => $collection_rate,
                'overdue_rate' => $overdue_rate,
                'credit_impact' => $credit_impact,
                'discount_dependency' => $discount_dependency,
                'net_revenue' => $net_sales // Consolidate
            ],
            'product_performance' => [
                 'gross_revenue' => $gross_product_revenue,
                 'credit_value' => $product_credit_value,
                 'net_revenue' => $net_product_revenue,
                 'net_units' => $net_units_sold,
                 'gross_profit' => $gross_product_profit,
                 'credit_loss' => $credit_profit_impact,
                 'net_profit' => $net_product_profit,
                 'net_margin_percent' => $net_margin_percent,
                 'credit_rate_percent' => $product_credit_rate
            ],
            'behavior' => [
                'frequency' => $purchase_frequency, 
                'recency' => $recency_days,
                'basket_size' => $avg_basket_size,
            ...
            'risk' => [
                'payment_risk' => $payment_risk_score, 
                'complaint_freq' => $complaint_frequency
            ],
            'credit_intelligence' => [
                'frequency_percent' => $credit_frequency_percent,
                'avg_value' => $avg_credit_value,
            ],
            'scores' => [
                'product_perf' => $prod_perf_score,
                'customer_perf' => $cust_perf_score,
                'sales_health' => $sales_health_score
            ],
            'predictive' => [
            ...
            'pipeline' => [
                'proposal_conv' => $proposal_conversion,
                'estimate_conv' => $estimate_conversion,
            ...
            'sales_counts' => [
                ...
                'credit_notes' => $count_credit_notes,
                'payments' => $count_payments,
                'items_invoiced' => $total_items_invoiced,
                'unique_items' => $total_unique_items
            ],
            'sales_amounts' => [
                'proposals' => $amount_proposals,
                'estimates' => $amount_estimates,
                'invoices' => $amount_invoices,
                'credit_notes' => $amount_credit_notes,
                'payments' => $amount_payments,
            ],
            'averages' => [
                'proposal' => $avg_proposal_val,
                'estimate' => $avg_estimate_val,
                'invoice' => $avg_invoice_val,
                'payment' => $amount_payments / ($count_payments > 0 ? $count_payments : 1)
            ]
        ];
    }
}