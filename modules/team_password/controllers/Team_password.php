       ...
       if ($this->input->is_ajax_request()) {
            if($this->input->post()){
              if($category != 'all'){
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_normal.id AND type = "normal") ))');
                }
    ...
    */
    ...
    public function bank_account_table($category)
    {
       if ($this->input->is_ajax_request()) {
            if($this->input->post()){
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_bank_account.id AND type = "bank_account") ))');
                }
    ...
    public function credit_card_table($category)
    {
       if ($this->input->is_ajax_request()) {
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_credit_card.id AND type = "credit_card") ))');
                }
    ...
    public function email_table($category)
    {
       if ($this->input->is_ajax_request()) {
            if($this->input->post()){
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_email.id AND type = "email") ))');
                }
       ...
       if ($this->input->is_ajax_request()) {
            if($this->input->post()){
               if($category != 'all'){
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_server.id AND type = "server") ))');
                }
    ...
    public function software_license_table( $category)
    {
       if ($this->input->is_ajax_request()) {
            if($this->input->post()){
                ...
                if(!has_permission('team_password','','view') && !is_admin()){
                  array_push($where, ' OR (add_from = '.get_staff_user_id().' OR '.get_staff_user_id().' IN (SELECT staff from '.db_prefix().'permission WHERE (obj_id = '.db_prefix().'tp_software_license.id AND type = "software_license") ))');
                }
